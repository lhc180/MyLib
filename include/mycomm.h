...
public:
  cBERtoEbN0(): setDone_(false){ }
  cBERtoEbN0(MODULATOR_TYPE modType)
  {
...
public:
  cRandomArrange1vec() : setDone_(false){ }
  cRandomArrange1vec(int allNum, int num1)
  {
    set(allNum, num1);
  }
...
// インターリーバを作る関数
void mylib::makeInterleaver(itpp::ivec &interleaver)
{
...
double mylib::erfc_inv(double x) 
{ 
...
itpp::bvec mylib::genErrors(const itpp::bvec &input, double errorProb)
{
  itpp::bvec output(input.size());
  cRand01 Rand01(errorProb);
...
itpp::bvec mylib::std_ivec2bvec(int length, const std::vector<int> &input)
{
  itpp::bvec output(0);
...
itpp::bvec mylib::itpp_ivec2bvec(int length, const itpp::ivec &input)
{
  itpp::bvec output(0);
...
std::vector<int> mylib::std_bvec2ivec(int length, const itpp::bvec &input)
{
...
itpp::ivec mylib::itpp_bvec2ivec(int length, const itpp::bvec &input)
{
  itpp::ivec output(0);
  for(int start = 0; start < input.size(); start += length){
    itpp::bvec t_bvec = input.mid(start, length);
